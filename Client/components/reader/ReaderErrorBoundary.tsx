...
/**
 * Unified Error Boundary for All Reader Types (PDF, EPUB, TXT)
 * Catches and handles errors in reader components, preventing full app crashes
 * Provides graceful degradation with error details and recovery options
 */
class ReaderErrorBoundary extends Component<Props, State> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo): void {
    logger.error('Reader Error Boundary caught an error:', error);
  ...
  getErrorSuggestions(): string[] {
    ...
    switch (fileType?.toLowerCase()) {
      case 'pdf':
        return [...commonSuggestions, 'Unsupported PDF features', 'Memory limitations'];
      case 'epub':
        return [...commonSuggestions, 'Invalid EPUB structure', 'Missing resources'];
      case 'txt':
        return [...commonSuggestions, 'File encoding issues', 'File too large'];
      default:
  ...
  render(): ReactNode {
    if (this.state.hasError) {
      ...
      const readerType = this.getFileTypeMessage();
        ...
        <div className="flex items-center justify-center min-h-screen bg-gray-50 dark:bg-gray-900 p-4">
          <div className="max-w-2xl w-full bg-white dark:bg-gray-800 rounded-lg shadow-xl p-8">
            ...
            <h1 className="text-2xl font-bold text-center text-gray-900 dark:text-gray-100 mb-4">
              Unable to Load {readerType}
            </h1>

            {/* Error Message */}
            <div className="mb-6">
              <p className="text-center text-gray-600 dark:text-gray-400 mb-4">
                Something went wrong while loading the reader. This could be due to:
              </p>
            ...
            <p className="text-center text-sm text-gray-500 dark:text-gray-400 mt-6">
              If this problem persists, please try a different file or contact support.
            </p>
          </div>
        </div>
      );
