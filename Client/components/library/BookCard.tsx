...
import { BookOpen, Bookmark, CheckCircle, MoreVertical, Trash2, Edit } from "lucide-react";
...
export default function BookCard({
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'want-to-read': return 'text-yellow-400';
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            {book.progress > 0 && (
              <span className="text-xs text-gray-400">• {Math.round(book.progress)}%</span>
            )}
          </div>
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-3 right-3 z-20 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-200">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm hover:bg-white dark:hover:bg-gray-800 shadow-sm">
              <MoreVertical className="w-4 h-4 text-gray-700 dark:text-gray-300" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-48 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700">
      ...
      >
        ...
        <div className="absolute bottom-3 right-3 z-10">
          <span className={`text-xs font-bold px-2 py-1 rounded-full backdrop-blur-md bg-black/30 ${getStatusColor(book.status)}`}>
            {getStatusLabel(book.status)}
          </span>
        </div>
      ...
      <div className="p-4 flex-1 flex flex-col">
        ...
        >
          <h3 className="font-bold text-gray-900 dark:text-white line-clamp-1" title={book.title}>
            {book.title}
          </h3>
        </Link>
        <p className="text-sm text-gray-500 dark:text-gray-400 line-clamp-1 mb-3" title={book.author || undefined}>
