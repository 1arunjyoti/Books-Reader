...
- **Auth**: Auth0 (JWT) protected API; per‑user data isolation.

**Tech Stack**
- **Client**: `Next.js 15`, `React 19`, `TypeScript`, `Tailwind CSS`, `React Query`, Radix UI/ShadCN.
- **Server**: `Express 5`, `Prisma`, `PostgreSQL`, `Backblaze B2 (S3)`, `Auth0`, `Winston`, `Zod`.
...
- **Prereqs**: Node.js 18+, PostgreSQL URL, Auth0 tenant, Backblaze B2 credentials.
...
# Edit .env with Auth0, DATABASE_URL (PostgreSQL), and B2 settings
...
# Set AUTH0_* and NEXT_PUBLIC_API_URL (http://localhost:5000 if using Option A)
npm install
npm run dev
```
